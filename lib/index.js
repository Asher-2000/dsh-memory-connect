...
import { mkdir, open, readFileSync, existsSync } from 'node:fs/promises'
 ...
 */
class SoulManager {
  static _soulContent = null
  static _lastLoaded = 0
  static _cacheMs = 5 * 60 * 1000 // 5 minutes cache

  /**
   * Load Soul content (cached for 5 minutes)
   * @returns {string|null} Soul content or null
   */
  static load() {
    const now = Date.now()
    if (this._soulContent !== null && (now - this._lastLoaded) < this._cacheMs) {
      return this._soulContent
    }

    try {
      const soulPath = join(process.env.HOME || '', '.dsh', 'soul.md')
      if (existsSync(soulPath)) {
        this._soulContent = readFileSync(soulPath, 'utf-8')
        this._lastLoaded = now
        return this._soulContent
      }
    } catch (error) {
      // Silently fail - Soul is optional
    }
   ...
   * @returns {string} Formatted Soul context
   */
  static getContext() {
    const soul = this.load()
    if (!soul) return ''

    return `## 🧠 Global Identity (Soul)
