...
function ensureMemorySchema(db) {
  ...
  // v3: embedding vectors for semantic recall (optional, populated when
  // the embedding service is reachable). Stored as raw float32 BLOB.
  db.exec(`
...
class SemanticConsolidator {
  ...
  async mergePair(pair) {
    ...
    // Archived memory's embedding is stale — drop it from semantic recall.
    this._db.prepare('DELETE FROM memory_embeddings WHERE memory_id = ?').run(archive.id)
...
/**
 * Thin HTTP client for a local embedding server (scripts/embed_server.py).
 * Lazily connects; caches the server's model/dim on first successful call.
 * All failures are non-fatal: semantic recall degrades to FTS-only.
 */
class EmbeddingClient {
  constructor(config) {
    this.baseUrl = config.embeddingUrl || 'http://127.0.0.1:8765'
    this.model = config.embeddingModel || 'BAAI/bge-small-zh-v1.5'
    this.enabled = !!config.embeddingEnabled
    this.weight = config.embeddingWeight ?? 0.7
    this._dim = 0
    this._lastError = null
    this._retryAt = 0
    this._timeoutMs = 15_000
  }
  ...
  /** Returns true when the server is reachable AND returned a valid dim. */
  get ready() {
    return this.enabled && this._dim > 0
  }

  _available() {
    if (!this.enabled) return false
    // Backoff: after a failure, wait 60s before trying again.
    if (Date.now() < this._retryAt) return false
    return true
  }
  ...
  async _request(path, payload) {
    ...
    try {
      ...
      if (!resp.ok) {
        throw new Error(`embedding server HTTP ${resp.status}`)
      }
  ...
  /** Health check; sets _dim on success. Never throws. */
  async ensureReady() {
  ...
  /**
   * Embed a batch of texts (non-blocking failure).
   * @param {string[]} texts
   * @returns {Promise<number[][]>} list of float arrays (dim = this._dim)
   */
  async embed(texts) {
    ...
    try {
      const data = await this._request('/embed', { texts })
...
export class CrossSessionMemoryService extends Service {
  ...
  static Config = z.object({
    ...
    soulPath: z.string(),
    /** Embedding: enable semantic recall via a local embedding HTTP server */
    embeddingEnabled: z.boolean().default(false),
    /** Embedding server base URL (default: http://127.0.0.1:8765) */
    embeddingUrl: z.string().default('http://127.0.0.1:8765'),
    /** Embedding server model (informational; must match the server's model) */
    embeddingModel: z.string().default('BAAI/bge-small-zh-v1.5'),
    /** RRF fusion weight for semantic results (0 = disabled, 1 = full weight) */
  ...
  _consolidator
  /** @type {EmbeddingClient | undefined} */
  _embedder
  ...
  constructor(ctx, config) {
    ...
    this._embedder = new EmbeddingClient(config)
  ...
  async searchMemories(request) {
    ...
    // Semantic ranking via embeddings (optional; RRF fusion with FTS)
    let semanticHits = []
    if (this._embedder?.enabled) {
      const qVec = await this._embedder.embed([request.query.slice(0, 500)])
      if (qVec?.length && this._embedder.dim > 0) {
        const allVecs = this._loadEmbeddings()
        const q = qVec[0]
        const scored = allVecs
          .map(ent => ({ id: ent.id, sim: this._cosine(q, ent.vec) }))
          .filter(x => x.sim > 0.35) // semantic similarity floor
          .sort((a, b) => b.sim - a.sim)
        // Exclude current session, apply type filter
        const allowed = new Set(
          db.prepare(`
            SELECT id FROM memories
            WHERE status = 'active' AND valid_until IS NULL
              ${request.excludeSessionId ? 'AND session_id != ?' : ''}
              ${request.types?.length ? `AND type IN (${request.types.map(() => '?').join(',')})` : ''}
          `).all(...(request.excludeSessionId ? [request.excludeSessionId] : []), ...(request.types || []))
            .map(r => r.id)
        )
        semanticHits = scored.filter(x => allowed.has(x.id)).slice(0, limit * 2)
      }
    }
    ...
    // Semantic results join the fusion with their own rank.
    const semWeight = this._embedder?.weight ?? 0.7
    for (let i = 0; i < semanticHits.length; i++) {
      const hit = semanticHits[i]
      const rrfScore = (1 / (k + i)) * semWeight
      if (rrfScores.has(hit.id)) {
        rrfScores.get(hit.id).score += rrfScore
      } else {
        const row = db.prepare('SELECT * FROM memories WHERE id = ?').get(hit.id)
        if (row) {
          rrfScores.set(hit.id, {
            memory: rowToMemory(row),
            score: rrfScore,
          })
        }
  ...
  recallSync(sessionId, query, limit) {
    try {
      ...
      // NOTE: semantic (embedding) recall lives in the async searchMemories
      // path — recallSync is synchronous and cannot await the HTTP embedder.
  ...
  async storeMemory(memory) {
    ...
    // Update embedding (async, best-effort)
    if (this._embedder?.enabled) {
      this._storeEmbedding(id, memory.content || '').catch(() => {})
    }
  ...
  async reviseMemory(oldId, updates = {}) {
    ...
    db.prepare(`
      ...
      WHERE id = ? AND status = 'active'
    `).run(now, oldId)
    // Old row's embedding is no longer active — remove it so recall ignores it.
    db.prepare('DELETE FROM memory_embeddings WHERE memory_id = ?').run(oldId)
    ...
    // Embedding for the successor (async, best-effort)
    if (this._embedder?.enabled) {
      this._storeEmbedding(newId, newContent).catch(() => {})
    }
  ...
  /** Convert float array to float32 BLOB (little-endian). */
  _vecToBlob(vec) {
  ...
  /** Convert float32 BLOB to float array. */
  _blobToVec(blob) {
    if (!blob) return null
    const buf = Buffer.isBuffer(blob) ? blob : Buffer.from(blob)
    if (buf.length % 4 !== 0) return null
    const vec = new Float64Array(buf.length / 4) // use Float64 for precision in JS math
    for (let i = 0; i < vec.length; i++) {
  ...
  /** Asynchronously compute + store embedding for a memory (non-blocking). */
  async _storeEmbedding(id, content) {
    if (!this._embedder?.enabled) return
    try {
      const vec = await this._embedder.embed([String(content || '').slice(0, 500)])
      if (!vec?.length) return
      const db = this._requireDb()
      db.prepare(`
        INSERT OR REPLACE INTO memory_embeddings (memory_id, embedding, model, dim, updated_at)
        VALUES (?, ?, ?, ?, ?)
      `).run(id, this._vecToBlob(vec[0]), this._embedder.model, this._embedder.dim, Date.now())
    } catch (error) {
      // Embedding is best-effort; never break storage on failure.
      console.error('[dsh-memory] embedding store error:', error?.message || error)
    }
  }

  /** Load all active embeddings for semantic scoring. */
  _loadEmbeddings() {
    const db = this._requireDb()
    const rows = db.prepare(`
      SELECT e.memory_id AS id, e.embedding AS blob, m.content AS content
      FROM memory_embeddings e
      JOIN memories m ON m.id = e.memory_id
      WHERE m.status = 'active' AND m.valid_until IS NULL
    `).all()
    const out = []
    for (const r of rows) {
      const vec = this._blobToVec(r.blob)
      if (vec?.length) out.push({ id: r.id, vec })
    }
    return out
  }
  ...
  /** Delete embedding for a memory (e.g. on supersede). */
  _deleteEmbedding(id) {
    const db = this._requireDb()
  ...
  async _open() {
    // Expand leading "~" to home directory (resolve() does NOT expand it).
    let rawPath = this.config.path
    if (typeof rawPath === 'string' && rawPath.startsWith('~/')) {
      rawPath = join(homedir(), rawPath.slice(2))
    }
    const path = rawPath === ':memory:'
      ? ':memory:'
      : resolve(rawPath)
